...
from gaussian_renderer import network_gui, vae_render_from_batch
import sys
...
def scene_reconstruction(dataset, opt, hyper, pipe, vae, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):   # 여기부터 iteration 시작   
        ...
        output = vae_render_from_batch(viewpoint_cams, gaussians, pipe, vae, random_color, stage=stage, batch_size=batch_size, canonical_tri_plane_factor_list=opt.canonical_tri_plane_factor_list,iteration=iteration)
        ...
        with torch.no_grad():
            ...
            # Log and save
            timer.pause()
            if iteration % 3000 == 0 or iteration == 15001:
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                scene.save(iteration, stage, torch.cat([gt_image_tensor, image_tensor]), viewpoint_cams[0].uid)
                vae_path = os.path.join(scene.model_path, "point_cloud/iteration_{}".format(iteration))
...
def training(accelerator, dataset, hyper, opt, pipe, testing_iterations, saving_iterations, 
    ...
    scene_reconstruction(dataset, opt, hyper, pipe, vae, testing_iterations, saving_iterations,
                             checkpoint_iterations, checkpoint, debug_from,
                             gaussians, scene, "fine", tb_writer, opt.coarse_iterations,timer, use_wandb, vgg_perceptual_loss, accelerator)
